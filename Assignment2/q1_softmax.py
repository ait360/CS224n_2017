...
def softmax(x):
    ...
    ### YOUR CODE HERE
    if len(x.shape) > 1:
        maximum = tf.reduce_max(x, axis=1, keep_dims=True)
        x_exp = tf.exp(x - maximum)
        out = x_exp / tf.reduce_sum(x_exp, axis=1, keep_dims=True)
    else:
        x_exp = tf.exp(x - tf.reduce_max(x, keep_dims=True))
        out = x_exp/tf.reduce_sum(x_exp, axis=0, keep_dims=True)
    ### END YOUR CODE
