...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        #print(sentence)
        for indx in range(len(labels)-1):
            a, b, c = sentence[indx: (indx + window_len)]
            #print(a)
            #print(b)
            #print(c)
            window_list = a + b + c
            windowed_data.append((window_list, labels[indx]))
        x, y, z = sentence[(len(sentence) - window_len) :]
        last_window_list = x + y + z
        windowed_data.append((last_window_list, labels[len(labels) - 1]))
