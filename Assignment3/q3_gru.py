...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        grads, var = list(zip(*grad_var_pair))
        if self.config.clip_gradients:
            grad, self.grad_norm = tf.clip_by_global_norm(grads,
                                                       clip_norm=self.config.max_grad_norm)
        else:
            self.grad_norm = tf.global_norm(grads)
        grad_var_pair = zip(grads, var)
        train_op = optimizer.apply_gradients(grad_var_pair)
