...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) > max_length:
            ...
            zero_len = max_length - len(sentence)
            old_len = len(sentence)
            sentence = sentence + ([zero_vector] * zero_len)
            labels = labels + [zero_label] * zero_len
            mask = ([True] * old_len) + ([False] * zero_len)
            ret.append((sentence, labels, mask))
        else:
            mask = [True] * max_length
            ret.append((sentence, labels, mask))
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                #print(U.shape, o_drop_t.shape, b_2.shape, sep='\n')
