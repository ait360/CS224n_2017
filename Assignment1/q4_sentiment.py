...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    sentVector = np.mean(wordVectors[word_index], axis=0)
...
def getRegularizationValues():
    ...
    ### YOUR CODE HERE
    values = [(np.exp(i)/400) for i in np.arange(0, 20, 2)] +  [(400 ** (1/i))for i in np.arange(1, 21, 2)]
    ### END YOUR CODE
...
def chooseBestModel(results):
    ...
    #    if result_dict["dev"] > bestDev:
    #        bestDev = result_dic["dev"]
    #        bestReuslt = result_dict
